 ...
 * auto result = __riscv_Add(img1, img2, CLIPPING_V::SATURATION);
 * // result = {{150, 250}, {230, 250}} (255 clamped)
 * ```
 */
vector<vector<uint8_t>> __riscv_Add(const vector<vector<uint8_t>>& image1, const vector<vector<uint8_t>>& image2 , CLIPPING_V clip);
 ...
 * auto result = __riscv_AddWeighted(img1, 0.7f, img2, 0.3f, 10.0f, CLIPPING_V::SATURATION);
 * // result = {{85, 135}, {149, 95}} (calculations: 0.7*100 + 0.3*50 + 10 = 85, etc.)
 * ```
 */
vector<vector<uint8_t>> __riscv_AddWeighted( const vector<vector<uint8_t>>& image1, float alpha, const vector<vector<uint8_t>>& image2, float beta, float gamma, CLIPPING_V clip);
 ...
 * vector<vector<uint8_t>> img2 = {{50, 100}, {30, 200}};
 * 
 * __riscv_Accumulate(accumulator, img1, CLIPPING_V::SATURATION);
 * // accumulator = {{100, 150}, {200, 50}} (first image copied)
 * 
 * __riscv_Accumulate(accumulator, img2, CLIPPING_V::SATURATION);
 * // accumulator = {{150, 250}, {230, 250}} (accumulated with saturation)
 * ```
 */
void __riscv_Accumulate (vector<vector<uint8_t>>& accumulator, const vector<vector<uint8_t>>& image, CLIPPING_V clip);
 ...
 * 
 * __riscv_AccumulateSquared(accumulator, img1, CLIPPING_V::SATURATION);
 * // accumulator = {{100, 225}, {144, 64}} (10²=100, 15²=225, etc.)
 * 
 * __riscv_AccumulateSquared(accumulator, img2, CLIPPING_V::SATURATION);
 * // accumulator = {{125, 255}, {193, 208}} (100+25=125, 225+100=255(saturated), etc.)
 * ```
 */
void __riscv_AccumulateSquared( vector<vector<uint8_t>>& accumulator, const vector<vector<uint8_t>>& image, CLIPPING_V clip);
