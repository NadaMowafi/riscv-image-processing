...
template <>
vuint8m1_t ImageFlipper_V<uint8_t>::create_reverse_index(size_t vl) {
...
template <typename T>
void ImageFlipper_V<T>::__riscv_flip(Image<T> &image, FlippingDirection_V direction)
{
...
template <>
void ImageFlipper_V<uint8_t>::__riscv_flipVertical(vector<vector<uint8_t>> &matrix)
{
...
template <>
void ImageFlipper_V<uint8_t>::__riscv_flipHorizontal(vector<vector<uint8_t>> &matrix)
{
    size_t height = matrix.size();
    size_t width = matrix[0].size();
