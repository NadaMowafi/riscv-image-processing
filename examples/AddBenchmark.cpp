...
int main() {
    ...
    auto scalar_fn = [&]() {
        scalarOut.pixelMatrix = add(
            image.pixelMatrix, image.pixelMatrix,
            CLIPPING::SATURATION);
    };
    auto vector_fn = [&]() {
         vectorOut.pixelMatrix = __riscv_Add(image.pixelMatrix, image.pixelMatrix, CLIPPING_V::SATURATION);
    };
